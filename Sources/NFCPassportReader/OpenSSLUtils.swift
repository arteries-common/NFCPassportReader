...
public class OpenSSLUtils {
    ...
    public static func computeSharedSecret( privateKeyPair: OpaquePointer, publicKey: OpaquePointer ) -> [UInt8] {
        ...
        if keyType == EVP_PKEY_DH || keyType == EVP_PKEY_DHX {
            ...
            let ctx_priv = OSSL_ENCODER_CTX_new_for_pkey(privateKeyPair, OSSL_KEYMGMT_SELECT_ALL, "DER", "type-specific", nil);


            var dh_priv = EVP_PKEY_new()
            var dh_priv_len = 0
            EVP_PKEY_derive(ctx_priv, &dh_priv, &dh_priv_len)


            let ctx_pub = OSSL_ENCODER_CTX_new_for_pkey(publicKey, OSSL_KEYMGMT_SELECT_ALL, "DER", "type-specific", nil);
            var dh_pub = EVP_PKEY_new()
            var dh_pub_len = 0
            EVP_PKEY_derive(ctx_pub, &dh_pub, &dh_pub_len)

            var bn_pub_key = BN_new()
            EVP_PKEY_get_bn_param(publicKey, OSSL_PKEY_PARAM_PUB_KEY, &bn_pub_key)

            secret = [UInt8](repeating: 0, count: dh_priv_len)
            //var sp = UnsafeMutablePointer(mutating: secret)
            // let len = OSSL_ENCODER_to_data(
            //     ctx_priv,
            //     &sp,
            //     &dh_priv_len
            // )
            let len = DH_compute_key(&secret, bn_pub_key, dh_priv);
